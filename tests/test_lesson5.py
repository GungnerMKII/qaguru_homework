...
def test_form():
    ...
    browser.element('#userName-wrapper #firstName').should(be.blank).type('Test')
    browser.element('#userName-wrapper #lastName').should(be.blank).type('Testov')
    browser.element('#userEmail-wrapper #userEmail').should(be.blank).type('Testov@test.com')
    browser.element("[for='gender-radio-1']").click()
    browser.element('#userNumber-wrapper #userNumber').should(be.blank).type('9628082744')
    browser.element('#dateOfBirth-wrapper #dateOfBirth').click()
